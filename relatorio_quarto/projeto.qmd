...
Foram desenvolvidos dois protótipos — um baseado em TF-IDF (abordagem estatística de ponderação lexical) e outro em *Sentence BERT* (SBERT) (arquitetura de *transformers* e *embeddings* densos) — avaliando a sua capacidade de gerar recomendações em dois cenários: o catálogo de 807 cursos da Escola Virtual de Governo (EVG) e uma coleção de 29.815 livros da Google Books API. Os resultados evidenciaram que ambos os modelos produzem sugestões relevantes, sendo o SBERT aquele que se destacou pela precisão semântica na captura de nuances contextuais (por exemplo, a distinção de polissemias e relações temáticas implícitas) e pela sua escalabilidade em situações de grande volume de dados, enquanto o TF-IDF mostrou-se preciso na identificação de correspondências lexicais entre palavras-chave. A aplicação prática nestes domínios demonstrou a viabilidade de implementação, com recomendações adaptáveis a diferentes necessidades. Este trabalho comprova que técnicas modernas de PLN podem democratizar o acesso a sistemas de recomendação, oferecendo soluções eficientes às mais diversas organizações, sem comprometer a qualidade das sugestões.
...
# Abstract {.unnumbered}
In a global context of massive information production, unstructured textual data—often overlooked or underutilised—pose an opportunity for organisations seeking to personalise the user experience without investing heavily in costly data infrastructures. Traditional recommender systems, particularly those based on collaborative filtering, require large volumes of behavioural data, making them almost prohibitively expensive for smaller organisations. This study therefore proposes an accessible yet effective alternative: recommender systems built entirely on textual content. To this end, Natural Language Processing (NLP) techniques were employed to analyse semantic similarity between text representation vectors with minimal computational overhead.

Two prototypes were developed—one based on TF-IDF (a statistical lexical weighting approach) and the other on Sentence BERT (SBERT) (a transformer architecture yielding dense embeddings)—and their recommendation capabilities were evaluated in two scenarios: the 807-course catalogue of the Escola Virtual de Governo (EVG) and a collection of 29,815 books from the Google Books API. Results show that both models generate relevant suggestions; SBERT stood out for its semantic precision in capturing contextual nuances (e.g., distinguishing polysemy and implicit thematic relations) and for its scalability in large-data environments, while TF-IDF proved accurate in identifying lexical matches between keywords. Practical implementation in these domains confirmed the feasibility of rapid deployment, with recommendations adaptable to diverse needs. This work demonstrates that modern NLP techniques can democratise access to recommender systems, offering efficient solutions to a wide range of organisations without compromising suggestion quality.
...
Quanto aos sistemas de recomendação desenvolvidos neste estudo, resta claro que os objetivos traçados na [Secção 2](#objetivos) foram alcançados com sucesso. Em primeira medida destacam-se os dois sistemas de recomendação desenvolvidos e testados: Um baseado em vectorização textual por meio de TF-IDF e o outro baseado em vectores de *embeddings* densos e arquitetura de transformers. 
