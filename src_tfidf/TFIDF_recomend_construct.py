...
class TfidfRecommender:
    ...
    def recomendar(self, id_item, top_n=3):
        ...
        recomendacao_cos = pd.DataFrame({
            'Item Recomendado': self.df_text['nome_original'].iloc[[i[0] for i in sim_cosine]],
            'Similaridade_Cosseno': [i[1] for i in sim_cosine]
        }).reset_index(drop=True)
        
        recomendacao_eucl = pd.DataFrame({
            'Item Recomendado': self.df_text['nome_original'].iloc[[i[0] for i in sim_euclidean]],
            'Distancia_Euclidiana': [i[1] for i in sim_euclidean]
        }).reset_index(drop=True)

        recomendacao_man = pd.DataFrame({
            'Item Recomendado': self.df_text['nome_original'].iloc[[i[0] for i in sim_manhattan]],
        ...
        return {
                ...
                "manhattan": recomendacao_man
            }


    
    ##FUNCIONA PERFEITAMENTE. ESSA FOI A UTILIZADA NO TRABALHO FINAL PARA COSINE APENAS
    # def recomendar(self, id_item, top_n=3):
